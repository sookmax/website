...
<StickyEventHeader id="medisola-feb-2025-update">
  <span className="not-prose">
    <a
      href="#medisola-feb-2025-update"
      className="relative after:content-['#'] after:invisible after:hover:visible after:absolute after:left-full after:ml-1 after:opacity-50"
    >
...
<div className="shadow-md shadow-zinc-200/70 dark:shadow-zinc-950/50 not-prose">
  ...
  >
    <a
      href="https://rd.archenx.com/"
      target="_blank"
      className="text-primary-dark"
    >
      {"https://rd.archenx.com/"}
    </a>
  </ImageGalleryInline>
</div>
...
As I was planning to wrap up the current sabbatical and start preparing for a job search soon, I wanted to go over one more subject presented in [Teach Yourself Computer Science](https://teachyourselfcs.com/). And [Designing Data-Intensive Applications (DDIA)](https://www.amazon.com/Designing-Data-Intensive-Applications-Reliable-Maintainable/dp/1449373321) is the book I picked up to read. Distributed systems like Hadoop and Spark were something that was very fascinating to me when I was a data analyst earlier in my career. In fact, they were so interesting that I went out of my way and built a mini hadoop cluster as [a side project](/experience#project-hadoop) around 2018. So yeah I was definitely excited to give this book a read.

Overall, I'd say the author did a really good job elaborating on various database distribution strategies such as replication and partitioning, and things that could go wrong in such distributed system. I also learned a lot about the internals of traditional databases such as database indexes, database transactions, and how they actually store data on file systems from this book. While reading, it definitely felt like this was something I should've read at least once when I was a data analyst (though it would've been hard to understand everything laid out in the book). It even explains terms like _OLAP_ and _data warehouses_ that I frequently heard but only vaguely understood in my data analyst days.

I'm not sure if I ever get a chance to work with distributed systems of a massive scale as a frontend developer, but I believe it still is beneficial for me to learn and be aware about these "under the hood" backend/infrastructure computing systems that power modern IT services that we all enjoy using.
...
Finally, it was time that I confronted [LeetCode](https://leetcode.com/) again, but this time at least I was better prepared, having just finished extensive data structures and algorithms (DSA) study for the first time in my life. In particular, I followed this wonderfully curated list of 150 LeetCode problems, called [NeetCode 150](https://neetcode.io/practice).

I would not dare to say I loved solving LeetCode problems even after learning DSA, but it was overall a good experience for me to review various algorithms and data structures I had just learned and to put into practice on actual implementations of said algorithms.
...
In the beginning of May. 2023, I finally decided to take a look at [Storybook](https://storybook.js.org/docs/react/get-started/why-storybook). I'd seen it being mentioned by many web devs on the internet quite frequently, and some would even argue Storybook is an indispensable tool when it comes to UI component development.

Well, they were certainly not wrong.

I still remember when I first grasped the concept of Storybook&mdash;in a nutshell, it provides you an isolated environment to develop a single UI component at a time without affecting other parts of UI or logics in your website. It felt like I found the final missing piece in my web development process when I discovered Storybook. It is a great tool in a sense that it's where all the individual UI components are going to be created, styled, assembled into bigger components, and tested thoroughly.

What's nice about developing a UI component in isolation is that you only need to worry about UI logics (as in how the component will behave) at the moment without having to think about stuff that needs to be handled by a framework, how to process data for this UI component, or various business logics right away&mdash;you can think about them later when you finish developing a functional UI component. This sort of separation of concerns, I believe, encourages your codebase to have a better structure&mdash;I find it almost always much harder to read the body of a React component in which all sorts of logics are intermingled together.

Anyways, the discovery of Storybook was so enlightening to me that I felt a strong desire to learn more about its internals, which led me to contribute to the project a little.
...
namely [tailwindcss](https://tailwindcss.com/) and [radix-ui](https://www.radix-ui.com/primitives), and so I simply wanted to write [stories](https://storybook.js.org/docs/react/writing-stories/introduction) about them 😄 (though at some point, it kind of got boring and I stopped working on it so it's only ~70% complete).
...
There was an online game that I used to enjoy playing&mdash;[Teamfight Tactics (TFT)](https://teamfighttactics.leagueoflegends.com/en-us/). So naturally I had been checking out [Riot's APIs](https://developer.riotgames.com/apis) to see if I could create something fun or useful for the game.
